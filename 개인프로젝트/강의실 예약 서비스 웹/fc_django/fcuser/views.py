...
# form 에 있던거 그대로 갖고와서 리팩토링 과정 (유효성 점검 부분이랑 데이터 저장부분을 나누어서 관리하는거)
    ...
    def form_valid(self, form):
        fcuser = Fcuser(
            ...
            # 이거 비밀번호 암호화하기싫다고 make_pasword안하면 로그인떄 오류남!!!!!!!!!!!!
            password=make_password(form.data.get('password')),
            level='user'
        )
        fcuser.save()
        messages.info(self.request, '일반등급으로 회원가입이 완료되었습니다.')
        return super().form_valid(form)


class RegisterViewForAdmin(FormView):
    template_name = 'register.html'
    form_class = RegisterForm
    success_url = '/'  # success_url 이라고 이것도 내장변수임 정상적으로 들어왔을때 특정 주소로 이동할수있게해줌!! 지금은 '/'니까 홈이동 의미
# form 에 있던거 그대로 갖고와서 리팩토링 과정 (유효성 점검 부분이랑 데이터 저장부분을 나누어서 관리하는거)
    ...
    def form_valid(self, form):
        fcuser = Fcuser(
            email=form.data.get('email'),
            # password=make_password(form.data.get('password')), 이거는 비밀번호 알수없게 하는 기능
            password=make_password(form.data.get('password')),
            level='admin'
        )
        fcuser.save()
        messages.info(self.request, '관리자등급으로 회원가입이 완료되었습니다.')
        return super().form_valid(form)
...
# @method_decorator(logout_message_required, name='dispatch') : 로그아웃했는데도 경고창 나옴.. 나중에 다시 체크
class AgreementView(View):
    ...
    def post(self, request, *args, **kwarg):
        if request.POST.get('agreement1', False) and request.POST.get('agreement2', False):
            ...
            if request.POST.get('student') == 'student':  # 뒤로가기 누른경우
                return redirect('/register/')
            else:  # 회원가입 버튼 누른경우
                return redirect('/registerforadmin/')
        else:
